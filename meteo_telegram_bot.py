...
def fetch_met_norway_weather(lat: float, lon: float, forecast_days: int = 3) -> dict:
    ...
    for item in series[:max_slots]:
        ...
        dt_local = dt_utc.astimezone(timezone(timedelta(hours=2))).replace(tzinfo=None)
        t_local_str = dt_local.strftime("%Y-%m-%dT%H:00")

        inst = item.get("data", {}).get("instant", {}).get("details", {})
        t = float(inst.get("air_temperature", 0.0))
        ...
        p = n1.get("details", {}).get("precipitation_amount", 0.0) if n1 else n6.get("details", {}).get("precipitation_amount", 0.0)
        p = max(0.0, float(p or 0.0))

        sym = (n1.get("summary", {}).get("symbol_code", "") or n6.get("summary", {}).get("symbol_code", "")).lower()
        if "thunder" in sym:
            code, prob = 95, 85.0
        elif "heavy" in sym:
            code, prob = 65, 90.0
        elif "rain" in sym:
            code = 63 if p >= 1.0 else 61
            prob = 75.0 if p >= 1.0 else 55.0
        elif "sleet" in sym:
            code, prob = 66, 60.0
        elif "snow" in sym:
            code, prob = 71, 70.0
            ...
            code, prob = 3, (15.0 if p == 0 else 45.0)
        elif "partly" in sym:
            code, prob = 2, 5.0
        elif "fair" in sym:
            code, prob = 1, 0.0
        ...
        if p > 0.1 and prob < 50:
            prob = min(95.0, 50.0 + p * 15.0)

        times.append(t_local_str)
        temps.append(t)
        precips.append(p)
        probs.append(prob)
    ...
    return {
        ...
        "_source": "MET Norway (Servizio Europeo)",
        "hourly": {
            "time": times,
...
def format_current_weather_message(data: Dict[str, Any]) -> str:
    ...
    out = [
        ...
        trend_text,
        "━━━━━━━━━━━━━━━━━━━━",
        f"🕒 <i>Rilevamento delle {updated_at} • {source_label}</i>"
    ]
